class MinangRuntime {
    ...
    initializeBuiltins() {
        ...
        // Gotong Royong - collaborative work
        this.functions.set('gotongRoyong', {
            type: 'builtin',
            arity: -1, // Variable arguments
            body: (args) => {
                console.log('🤝 Gotong Royong: Bekerja sama untuk', ...args.map(arg => this.toString(arg)));
                return args.reduce((acc, val) => {
                    if (typeof acc === 'number' && typeof val === 'number') {
                        return acc + val;
                    }
                    return this.toString(acc) + ' ' + this.toString(val);
                }, '');
            }
        });

        // Musyawarah - consensus building
        this.functions.set('musyawarah', {
            type: 'builtin',
            arity: 2,
            body: (args) => {
                const [topic, participants] = args;
                console.log(`🗣️ Musyawarah tentang: ${this.toString(topic)}`);
                console.log(`👥 Peserta: ${this.toString(participants)}`);
                return `Mufakat tercapai untuk ${this.toString(topic)}`;
            }
        });

        // Alam Takambang Jadi Guru - learning from nature
        this.functions.set('alamTakambang', {
            type: 'builtin',
            arity: 1,
            body: (args) => {
                const [lesson] = args;
                console.log(`🌿 Alam Takambang Jadi Guru: ${this.toString(lesson)}`);
                return `Pelajaran dari alam: ${this.toString(lesson)}`;
            }
        });

        // Adat Basandi Syarak - ethical practices
        this.functions.set('adatBasandi', {
            type: 'builtin',
            arity: 1,
            body: (args) => {
                const [action] = args;
                console.log(`⚖️ Adat Basandi Syarak: Memastikan ${this.toString(action)} sesuai etika`);
                return true; // Always return true for ethical actions
            }
        });
